...
const newPackage = (data) => {
  ...
  try {
    // Verificar que la cantidad de campos en data sea la esperada
    const expectedFields = 18;
    const divided = data.split(",");
    if (divided.length !== expectedFields) {
      console.error(
        `Data no contiene la cantidad de campos esperados. Se esperaban ${expectedFields} campos, pero se encontraron ${divided.length} campos.`
      );
      return data; // O simplemente retornar data si prefieres mantener el paquete sin procesar
    }
    ...
    console.error("Error en la función newPackage:", error);
    throw error; // Relanzamos el error para que se maneje en la función que llama a newPackage
  }
};
...
function sendViaNetcat(data) {
  try {
    ...
    console.error("Error en la función sendViaNetcat:", error);
    throw error; // Relanzamos el error para que se maneje en la función que llama a sendViaNetcat
  }
}
